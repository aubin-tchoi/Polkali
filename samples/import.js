...
function importation() {
  ...
  if (sourceURL.getSelectedButton() == ui.Button.CANCEL || sourceURL.getResponseText() == "") {
    return;
  }

  let sourcename = ui.prompt("Import de données", "Entrez le nom de l'onglet à importer :", ui.ButtonSet.OK_CANCEL);
  if (sourcename.getSelectedButton() == ui.Button.CANCEL || sourcename.getResponseText() == "") {
    return;
  }
  ...
  if (headerpos.getSelectedButton() == ui.Button.CANCEL || headerpos.getResponseText() == "") {
    return;
  }
  ...
  if (columns.length != 1) {
    ui.alert("Sélection non valide", "Veuillez recommencer en sélectionnant exactement 1 ligne", ui.ButtonSet.OK);
    return;
  }
